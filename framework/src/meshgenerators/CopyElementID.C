...
#include "CopyElementID.h"

#include "libmesh/elem.h"

registerMooseObject("MooseApp", CopyElementID);

InputParameters
CopyElementID::validParams()
{
  InputParameters params = MeshGenerator::validParams();
  params.addRequiredParam<MeshGeneratorName>("input", "The mesh we want to modify");
  params.addRequiredParam<std::string>("source_element_id", "The element ID to be copied");
  params.addRequiredParam<std::vector<std::string>>("target_element_ids", "The target element IDs");
  params.addClassDescription("Copy element ID to other element IDs.");
  return params;
}

CopyElementID::CopyElementID(const InputParameters & params)
  : MeshGenerator(params), _input(getMesh("input"))
{
}

std::unique_ptr<MeshBase>
CopyElementID::generate()
{
  ...
  auto target_id_names = getParam<std::vector<std::string>>("target_element_ids");
