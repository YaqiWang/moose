...
registerMooseObject("MooseTestApp", VolumeMaterial);

InputParameters
VolumeMaterial::validParams()
{
...
VolumeMaterial::VolumeMaterial(const InputParameters & parameters) :
    Material(parameters),
...
void
VolumeMaterial::computeQpProperties()
{
  if (!_bnd)
  {
    for (unsigned int i=0; i<_num_props; i++)
    ...
    mooseError("this volumetric material should not be called on faces!");
}
