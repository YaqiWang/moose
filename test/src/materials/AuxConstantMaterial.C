...
AuxConstantMaterial::AuxConstantMaterial(const std::string & name, InputParameters parameters) :
    Material(name, parameters),
    _prop_names(getParam<std::vector<std::string> >("prop_names")),
    _prop_values(getParam<std::vector<Real> >("prop_values"))
{
  _added_prop = &getMaterialProperty<Real>(getParam<std::string>("added_material_property"));
...
void
AuxConstantMaterial::computeQpProperties()
{
  for(unsigned int i=0; i<_num_props; i++)
    (*_properties[i])[_qp] = _prop_values[i] + (*_added_prop)[_qp];
}
